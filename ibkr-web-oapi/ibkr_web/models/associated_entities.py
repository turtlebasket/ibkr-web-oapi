from typing import TYPE_CHECKING, Any, Dict, List, Type, TypeVar, Union

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

if TYPE_CHECKING:
    from ..models.associated_entity import AssociatedEntity
    from ..models.associated_individual import AssociatedIndividual


T = TypeVar("T", bound="AssociatedEntities")


@_attrs_define
class AssociatedEntities:
    """
    Attributes:
        associated_individuals (Union[Unset, List['AssociatedIndividual']]):
        associated_entities (Union[Unset, List['AssociatedEntity']]):
    """

    associated_individuals: Union[Unset, List["AssociatedIndividual"]] = UNSET
    associated_entities: Union[Unset, List["AssociatedEntity"]] = UNSET
    additional_properties: Dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> Dict[str, Any]:
        associated_individuals: Union[Unset, List[Dict[str, Any]]] = UNSET
        if not isinstance(self.associated_individuals, Unset):
            associated_individuals = []
            for associated_individuals_item_data in self.associated_individuals:
                associated_individuals_item = associated_individuals_item_data.to_dict()
                associated_individuals.append(associated_individuals_item)

        associated_entities: Union[Unset, List[Dict[str, Any]]] = UNSET
        if not isinstance(self.associated_entities, Unset):
            associated_entities = []
            for associated_entities_item_data in self.associated_entities:
                associated_entities_item = associated_entities_item_data.to_dict()
                associated_entities.append(associated_entities_item)

        field_dict: Dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if associated_individuals is not UNSET:
            field_dict["associatedIndividuals"] = associated_individuals
        if associated_entities is not UNSET:
            field_dict["associatedEntities"] = associated_entities

        return field_dict

    @classmethod
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        from ..models.associated_entity import AssociatedEntity
        from ..models.associated_individual import AssociatedIndividual

        d = src_dict.copy()
        associated_individuals = []
        _associated_individuals = d.pop("associatedIndividuals", UNSET)
        for associated_individuals_item_data in _associated_individuals or []:
            associated_individuals_item = AssociatedIndividual.from_dict(associated_individuals_item_data)

            associated_individuals.append(associated_individuals_item)

        associated_entities = []
        _associated_entities = d.pop("associatedEntities", UNSET)
        for associated_entities_item_data in _associated_entities or []:
            associated_entities_item = AssociatedEntity.from_dict(associated_entities_item_data)

            associated_entities.append(associated_entities_item)

        associated_entities = cls(
            associated_individuals=associated_individuals,
            associated_entities=associated_entities,
        )

        associated_entities.additional_properties = d
        return associated_entities

    @property
    def additional_keys(self) -> List[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
